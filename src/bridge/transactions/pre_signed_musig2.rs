use bitcoin::{
  taproot::TaprootSpendInfo, PublicKey, TapSighashType
};
...
use super::{
  ...
  signing::push_taproot_leaf_script_and_control_block_to_witness, signing_musig2::{generate_nonce, get_aggregated_nonce, get_aggregated_signature, get_partial_signature},
};
...
pub fn push_nonce<T: PreSignedTransaction + PreSignedMusig2Transaction>(tx: &mut T, context: &VerifierContext, input_index: usize) -> SecNonce {
...
pub fn pre_sign_musig2_taproot_input<T: PreSignedTransaction + PreSignedMusig2Transaction>(
  ...
  let musig2_nonces = &tx.musig2_nonces()[&input_index].values().map(|public_nonce| public_nonce.clone()).collect();
...
pub fn finalize_musig2_taproot_input<T: PreSignedTransaction + PreSignedMusig2Transaction>(
  ...
  let script = &tx.prev_scripts()[input_index].clone();
  let musig2_nonces = &tx.musig2_nonces()[&input_index].values().map(|public_nonce| public_nonce.clone()).collect();
  let musig2_signatures = tx.musig2_signatures()[&input_index].values()
