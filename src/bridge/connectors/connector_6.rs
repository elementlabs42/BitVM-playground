...
pub struct Connector6 {
    ...
    pub evm_txid: String,
    pub peg_out_txid: Txid,
}

impl Connector6 {
    pub fn new(network: Network, operator_public_key: &PublicKey, evm_txid: &str, peg_out_txid: Txid) -> Self {
      Connector6 {
            network,
            operator_public_key: operator_public_key.clone(),
            evm_txid: evm_txid.to_string(),
            peg_out_txid,
        }
    }
}
...
impl P2wshConnector for Connector6 {
    fn generate_script(&self) -> ScriptBuf {
      script! {
        OP_FALSE
        OP_IF
        { self.evm_txid.clone().into_bytes() }
        OP_ENDIF
        OP_FALSE
        OP_IF
        { self.peg_out_txid.clone().to_string().into_bytes() }
        OP_ENDIF
