...
async fn test_take1_tx() {
    ...
    let funding_utxo_address0 =
        generate_pay_to_pubkey_script_address(&context.n_of_n_public_key.unwrap(), Network::Testnet);
    ...
    let funding_utxo_address1 =
        generate_timelock_script_address(&context.n_of_n_public_key.unwrap(), 2, Network::Testnet);
