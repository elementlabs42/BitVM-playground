...
use bitvm::{execute_script, signatures::winternitz_compact::{digits_to_number, checksig_verify, sign, N0_32, N1_32}};
...
async fn test_winternitz_compact_u32_signature_success() {
  ...
  println!("signature: {:?}", sign::<N0_32, N1_32>(&secret_key, message).compile());
  println!("locking_script: {:?}", checksig_verify::<N0_32, N1_32>(secret_key).compile());
  let script = script!{
    { signature }
    { locking_script }
